...
@tree.command(name="new-bets", description="Brand-new positions opened this quarter across all followed funds")
async def cmd_new_bets(interaction: discord.Interaction):
    if not _in_ch(interaction):
        await interaction.response.send_message("Wrong channel.", ephemeral=True)
        return
    try:
    ...
    for fund in funds:
        ...
        fund_name = fd.get("name") or fund["name"]
        quarter   = fd.get("latest_quarter", "?")

        if not history:
            continue  # no previous quarter to compare against

        prev_cusips = set(history[0]["positions"].keys())
        for cusip, pos in positions.items():
            if cusip not in prev_cusips and pos.get("value_usd", 0) >= config.MIN_POSITION_VALUE_USD:
                ticker = pos.get("ticker") or pos.get("name", "?")[:12]
                new_bets.append({
                    "ticker":    ticker,
                    "fund":      fund_name,
                    "value":     pos["value_usd"],
    ...
    if not new_bets:
        await interaction.followup.send(
            "No new positions detected yet. TINA needs at least two quarters of data per fund to show new bets."
        )
        return

    new_bets.sort(key=lambda x: x["value"], reverse=True)
    lines = [
        f"**${b['ticker']}** — {b['fund']} · {_fmt_usd(b['value'])} · {b['quarter']}"
        for b in new_bets[:25]
    ]
    await interaction.followup.send(
        f"**🆕 New Bets This Quarter — {len(new_bets)} new position(s):**\n" + "\n".join(lines)
    )
...
@tree.command(name="emerging", description="Tickers where multiple institutions independently opened positions this quarter")
@app_commands.describe(
    min_funds="How many institutions must have entered independently (default: 2)",
)
async def cmd_emerging(interaction: discord.Interaction, min_funds: int = 2):
    if not _in_ch(interaction):
        await interaction.response.send_message("Wrong channel.", ephemeral=True)
        return
    try:
    ...
    funds = _load_funds()
    if not funds:
        await interaction.followup.send("No funds followed yet. Use `/follow` to start.")
        return

    # Recompute from state diffs across all funds (always accurate, no API calls)
    ticker_quarter: dict[str, dict] = {}  # "{ticker}:{period}" → {funds: [...], total_value}
    ...
    for f in funds:
        ...
        for ev in events:
            ...
            existing = ticker_quarter[key]["entries"]
    ...
    min_funds = max(2, min_funds)
    consensus = [
        v for v in ticker_quarter.values()
        if len(v["entries"]) >= min_funds
    ]

    if not consensus:
        await interaction.followup.send(
            f"No tickers with {min_funds}+ funds entering in the same quarter yet.\n"
            "TINA needs at least two quarters of data per fund to detect ENTER events. "
            "Lower `min_funds` or follow more funds."
        )
        return
    ...
    for c in consensus[:15]:
        ...
        fund_parts = " · ".join(
            f"{e['fund'][:16]} {_fmt_usd(e['value'])} ({e['weight_pct']:.2f}%)"
            for e in sorted(c["entries"], key=lambda x: x["value"], reverse=True)
        )
        lines.append(
            f"**${c['ticker']}** — {len(c['entries'])} funds · {_fmt_usd(total)} combined · {c['quarter']}\n"
            f"  ↳ {fund_parts}"
        )
    ...
    await interaction.followup.send(embed=discord.Embed(
        ...
        description=(
            f"Tickers where {min_funds}+ followed funds independently opened positions "
            f"in the same quarter:\n\n" + "\n\n".join(lines)
        ),
        color=0x9b59b6,
    ).set_footer(text="TINA — independent entries in same quarter · not just co-ownership"))


# ── /entry ───────────────────────────────────────────────────────────────────

@tree.command(name="entry", description="Cost-basis overlay — were you early or chasing on recent institutional entries?")
@app_commands.describe(
    fund="Fund to analyse (leave blank for all followed funds)",
    ticker="Specific ticker to check (optional — leave blank for all new positions)",
)
async def cmd_entry(interaction: discord.Interaction, fund: str = "", ticker: str = ""):
    if not _in_ch(interaction):
        await interaction.response.send_message("Wrong channel.", ephemeral=True)
        return
    try:
    ...
    if fund:
        ...
        if not funds:
            await interaction.followup.send(f"No fund matching **{fund}**.")
            return
    ...
    for f in funds:
        ...
        fund_name = st.get("funds", {}).get(cik, {}).get("name") or f["name"]
        events    = flow.diff_from_state(st, cik, fund_name)
        enters    = [e for e in events if e.type in ("ENTER", "ADD") and e.ticker]
        if ticker:
            enters = [e for e in enters if e.ticker.upper() == ticker.upper()]
        all_enters.extend(enters)

    if not all_enters:
        label = f" for **${ticker.upper()}**" if ticker else ""
        await interaction.followup.send(
            f"No new positions{label} found. TINA needs at least two quarters of data per fund."
        )
        return

    all_enters.sort(key=lambda e: e.value_usd, reverse=True)
    to_enrich = all_enters[:8]

    await interaction.followup.send(
        f"Fetching price history for **{len(to_enrich)}** position(s)… (~{len(to_enrich) * 2}s)"
    )

    loop = asyncio.get_event_loop()
    await loop.run_in_executor(
        None,
        lambda: flow.enrich_cost_basis(to_enrich, limit=len(to_enrich), delay=0.3),
    )

    enriched   = [e for e in to_enrich if e.cost_basis]
    unenriched = [e for e in to_enrich if not e.cost_basis]
    ...
    for e in enriched:
        ...
        lines.append(
            f"**${e.ticker}** — {e.fund_name} · {e.type} · {_fmt_usd(e.value_usd)} ({e.weight_pct:.2f}% of book)\n"
            f"  Entry range {e.quarter}: **${cb['low']:.2f}–${cb['high']:.2f}** · avg **${cb['avg_close']:.2f}**\n"
            f"  Now **${cb['current_price']:.2f}** ({sign}{cb['pct_from_avg']:.0f}% from avg) — {verdict}"
        )

    if unenriched:
        lines.append(
            f"\n*No price data: {', '.join('$' + e.ticker for e in unenriched)}*"
        )

    await interaction.followup.send(embed=discord.Embed(
        title="📐 Entry Analysis — Are You Early or Chasing?",
        description="\n\n".join(lines) or "No enriched results.",
        color=config.COLOR_INFO,
    ).set_footer(text="Entry range = OHLC during the 13F reporting quarter · TINA"))


@cmd_entry.autocomplete("fund")
async def entry_fund_ac(_interaction: discord.Interaction, current: str):
    return _followed_ac(current)


# ── /exits ───────────────────────────────────────────────────────────────────

@tree.command(name="exits", description="Positions closed or heavily trimmed last quarter across followed funds")
@app_commands.describe(
    fund="Specific fund to check (leave blank for all followed funds)",
    type="Show exits only, trims only, or both (default: both)",
)
@app_commands.choices(type=[
    app_commands.Choice(name="Both exits and trims", value="both"),
    app_commands.Choice(name="Exits only (full closes)", value="exit"),
    app_commands.Choice(name="Trims only (partial reduces)", value="trim"),
])
async def cmd_exits(
    interaction: discord.Interaction,
    fund: str = "",
    ...
    # Filter to a single fund if specified
    if fund:
        cik_filter = _resolve_cik(fund)
        funds = [f for f in funds if f["cik"] == cik_filter]
        if not funds:
            await interaction.followup.send(f"No fund matching **{fund}**. Use `/funds` to see what's followed.")
            return
    ...
    if type == "exit":
        filtered = [e for e in all_events if e.type == "EXIT"]
    elif type == "trim":
        filtered = [e for e in all_events if e.type == "TRIM"]
    else:
        filtered = [e for e in all_events if e.type in ("EXIT", "TRIM")]

    if not filtered:
        label = {"exit": "exits", "trim": "trims", "both": "exits or trims"}[type]
        await interaction.followup.send(
            f"No notable {label} found. Either there's only one quarter of data "
            "per fund so far, or no positions were closed/trimmed last quarter."
        )
        return

    filtered.sort(key=lambda e: (e.prev_value_usd, e.score), reverse=True)

    lines = []
    for e in filtered[:20]:
        ticker   = f"${e.ticker}" if e.ticker else e.name[:15]
        ref_val  = e.prev_value_usd
        wt_str   = f"{e.prev_weight_pct:.2f}% of book"
        if e.type == "EXIT":
            lines.append(f"🚪 **{ticker}** — {e.fund_name} fully exited · was {_fmt_usd(ref_val)} ({wt_str})")
        else:
            lines.append(
                f"➖ **{ticker}** — {e.fund_name} trimmed {abs(e.delta_pct):.0f}% · "
                f"{_fmt_usd(e.prev_value_usd)} → {_fmt_usd(e.value_usd)} ({wt_str} → {e.weight_pct:.2f}%)"
            )

    exits_n = sum(1 for e in filtered if e.type == "EXIT")
    trims_n = sum(1 for e in filtered if e.type == "TRIM")
    header  = f"🚪 {exits_n} exit(s) · ➖ {trims_n} trim(s)" if type == "both" else f"{len(filtered)} result(s)"

    await interaction.followup.send(embed=discord.Embed(
        title="📤 Smart Money Leaving",
        description=f"{header} across {len(funds)} fund(s)\n\n" + "\n".join(lines),
        color=config.COLOR_EXIT,
    ).set_footer(text="Based on latest vs. previous 13F quarter · TINA"))


@cmd_exits.autocomplete("fund")
async def exits_fund_ac(_interaction: discord.Interaction, current: str):
    return _followed_ac(current)
